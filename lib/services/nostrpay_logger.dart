...
final Logger _logger = Logger('NostrpayLogger');

class NostrpayLogger {
  NostrpayLogger() {
    ...
    //     deviceInfo.data.forEach(
    //       (String key, dynamic value) => _logger.info('$key: $value'),
    //     );
    //   },
  ...
  void _createSessionLogFile() async {
    try {
      ...
      final Directory appDir = Directory(config.workingDir);

      _pruneLogs(appDir);

      final File logFile = File(
        '${appDir.path}/logs/${DateTime.now().millisecondsSinceEpoch}.app.log',
      );
  ...
  String _recordToString(LogRecord record) {
    final String header =
        '[${record.loggerName}] {${record.level.name}} (${_formatTime(record.time)})';
